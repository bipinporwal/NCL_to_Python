...
def calculate_monthly_values(x, arith, nDim, opt):
    ...
    # Get the num_of_dim of dataarray
    len_of_dim = x.sizes
    num_of_dim = len(len_of_dim)
    
    # Check if the num_of_dim of dim is valid or invalid; if invalid, exit the function
    if (num_of_dim > 5):
        print("calculate_monthly_values: number of dimension = {} [only 5 dimensions or fewer supported]".format(num_of_dim))
        return None
